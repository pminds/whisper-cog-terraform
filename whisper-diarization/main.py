...
async def predict(request: Request, predict_request: PredictRequest):
    ...
    try:
        ...
        file_from_json = input_data.get("file")  # If 'file' is a URL, handle it like file_url
        ...
        if file_url or file_from_json:
            ...
            headers = {
                "User-Agent": "FastAPI-File-Downloader"
            }
            ...
            raise HTTPException(status_code=400, detail="Either 'file', 'file_url', or uploaded file must be provided")
        ...
        raise HTTPException(status_code=400, detail="Error downloading file: " + str(req_err))
...
def speech_to_text(
    ...
    logger.debug("Transcription completed in %.5f seconds", time_transcribing_end - time_start)
    ...
    logger.debug("Diarization completed in %.5f seconds", time_diraization_end - time_transcribing_end)
    ...
    logger.debug("Merging completed in %.5f seconds", time_merging_end - time_diraization_end)
    ...
    logger.debug("Cleaning completed in %.5f seconds", time_cleaning_end - time_merging_end)
    time_end = time.time()
